...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    if (orgAction !== 'create' && orgAction !== 'join') {
      return res.status(400).json({ message: 'Please specify whether you are creating or joining an agency.' });
    }
    ...
    if (orgAction === 'create') {
      if (!organizationName || !organizationName.trim()) {
        return res.status(400).json({ message: 'Please enter a name for your agency.' });
      }
      ...
      if (!inviteCode || !inviteCode.trim()) {
        return res.status(400).json({ message: 'Please enter your agency\'s invite code.' });
      }
      if (!role) {
        return res.status(400).json({ message: 'Please select your role — it is required.' });
      }
      organization = await Organization.findOne({ inviteCode: inviteCode.trim().toUpperCase() });
      if (!organization) {
        return res.status(404).json({ message: 'That invite code doesn\'t match any agency. Double-check it with your team.' });
      }
      effectiveRole = role;
    }
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    if (!organization) {
      return res.status(500).json({ message: 'Your account is not linked to a valid agency. Contact support.' });
    }
...
app.put('/organizations/me', requireAuth, requireRole(...MANAGEMENT), async (req, res) => {
  try {
    ...
    if (req.body.regenerateInviteCode) {
    ...
    res.json({
      ...
      id: org._id, name: org.name, slug: org.slug, inviteCode: org.inviteCode, memberCount,
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
app.get('/api/dashboard', requireAuth, async (req, res) => {
  try {
    ...
    const openMaintenance = maintenance
      .filter(m => m.status === 'Open' || m.status === 'In Progress').length;
    ...
    res.json({
      user: {
        id: me._id, firstName: me.firstName, lastName: me.lastName,
        email: me.email, phone: me.phone, role: me.role, avatar: me.avatar,
      },
      organization: {
        id: org._id, name: org.name, type: org.type,
        email: org.email, phone: org.phone, location: org.location, address: org.address,
        inviteCode: MANAGEMENT.includes(req.user.role) ? org.inviteCode : undefined,
      },
      stats: {
        ...
        occupancyRate:        totalUnits ? Math.round((occupiedUnits / totalUnits) * 1000) / 10 : 0,
      },
      charts: {
        propertyBreakdown,
...
app.get('/api/organization', requireAuth, async (req, res) => {
  ...
  if (!org) return res.status(404).json({ message: 'Organization not found' });
  const payload = {
    id: org._id, name: org.name, slug: org.slug, type: org.type,
    email: org.email, phone: org.phone, location: org.location, address: org.address,
  };
  if (MANAGEMENT.includes(req.user.role)) payload.inviteCode = org.inviteCode;
  res.json(payload);
...
app.get('/api/properties', requireAuth, async (req, res) => {
  try {
    ...
    const enriched = await Promise.all(props.map(async p => {
      ...
      const expected = units.reduce((s, u) => s + (u.price || 0), 0);
      return {
        ...p.toObject(),
...
app.get('/api/properties/:id/units', requireAuth, async (req, res) => {
  try {
    ...
    const units = await Unit.find(orgScope(req, { property: property.name })).sort({ name: 1 });
    res.json(units);
  } catch (err) {
...
app.put('/api/properties/:id', requireAuth, requireRole(...MANAGEMENT, 'leasing-agent'), async (req, res) => {
  try {
    ...
    const updated = await Property.findOneAndUpdate(
      orgScope(req, { _id: req.params.id }), update, { new: true }
    );
  ...
  try {
    ...
    if (unitId) {
      ...
      if (property && unit.property && unit.property !== property.name) {
        return res.status(400).json({ message: 'Selected unit does not belong to the chosen property.' });
      }
      if (unit.tenantId) {
        return res.status(409).json({ message: `Unit ${unit.name} is already occupied.` });
      }
    }
...
async function resolveTenantAssignment(req) {
  ...
  if (!tenantUser) {
    const err = new Error('Selected tenant account was not found in your organization.');
    err.status = 400;
    throw err;
  }
  return {
...
app.get('/maintenance', requireAuth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'tenant') {
      ...
      if (unit) {
        orClauses.push({ property: unit.property, unit: unit.name });
      }
      query = orgScope(req, { $or: orClauses });
    }

    const orders = await Maintenance.find(query).sort({ createdAt: -1 });
    res.json(orders);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
async function resolveStaffAssignment(req) {
  ...
  if (!staffUser) {
    const err = new Error('Selected staff account was not found in your organization.');
    err.status = 400;
    throw err;
  }
  return {
    assignedToId: staffUser._id,
    assignedTo: `${staffUser.firstName} ${staffUser.lastName}`.trim(),
  };
}
...
app.post('/maintenance', requireAuth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'tenant') {
      ...
      if (!unit) {
        return res.status(400).json({
          message: 'You are not currently assigned to a unit. Please contact your property manager.'
        });
      }
    ...
    const order = new Maintenance({
      ...
      status:      forcedStatus,
      assignedTo:  assignment.assignedTo || req.body.assignedTo || '',
      assignedToId: 'assignedToId' in assignment ? assignment.assignedToId : null,
      description: req.body.description || '',
    });
  ...
  try {
    ...
    if (!unit) {
      return res.json({
        ...
        property: null,
        rent: 0,
        currentMonth: { monthLabel: '', due: 0, paid: 0, balance: 0, status: 'No Unit' },
        payments: [],
        maintenance: { open: 0, total: 0 },
        charts: {
          paymentsByMonth: [],
          maintByStatus: { Open: 0, 'In Progress': 0, Resolved: 0, Closed: 0 },
        },
      });
    }

    const payments = await Payment.find(orgScope(req, {
      $or: [
        { tenantId: me._id },
    ...
    payments.forEach(p => {
      const pd = p.date ? new Date(p.date) : new Date(p.createdAt);
      if (!pd) return;
      const m = paymentsByMonth.find(x => x.year === pd.getFullYear() && x.month === pd.getMonth());
      if (!m) return;
      if (p.status === 'Paid') m.paid += (p.amount || 0);
      else if (p.status === 'Pending' || p.status === 'Overdue') m.pending += (p.amount || 0);
    });
...
app.get('/api/tenant/receipts', requireAuth, requireRole('tenant'), async (req, res) => {
  try {
    ...
    const receipts = await Payment.find(orgScope(req, {
      status: 'Paid',
      $or: [
        { tenantId: me._id },
        { tenantId: null, tenant: fullName },
